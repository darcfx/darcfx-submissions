...
sub ModifyPost {
	...
	if (($Group ne "administrator")&&($Group ne $BoardInfo[4])) {
        ...
        if ($Group eq "Guest") {
            print "Location: UltraBoard.$Ext?Action=SignIn&Ref=ModifyPost&Board=$in{'Board'}&Post=$in{'Post'}&ID=$in{'ID'}&Idle=$in{'Idle'}&Sort=$in{'Sort'}&Order=$in{'Order'}&Page=$in{'Page'}\n\n";
...
###############################################################################
	...
	if (($PostInfo[2]) && (!$PostInfo[1])) {
		...
		$HTML.=	"<b>$PostInfo[1] ($PostInfo[2])</b>";
	}
	$HTML.=	        "</font>".
                    "</td>".
				"</tr>".
		...
		$HTML.=	&Tr("","",$CategoryBGColor).
				...
				&Tr("","",$RowOddBGColor).
					...
					&Td("100%","","","","","","",$ColumnEvenBGColor,"").
						&Font($FontFace,$TextSize,$TextColor).
							"<b>Nodify you by email when someone reply this topic?</b>".
						"</font>".
					"</td>".
				"</tr>".
				&Tr("","",$RowEvenBGColor).
	...
	$HTML.=	    &Tr("","",$CategoryBGColor).
					&Td("","","2","","","","","","").
						"<center>".&Submit("","MODIFY THIS TOPIC","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","2","","","","","","").
			...
			"</table></td></tr></table>".
			"</form>";
	&PrintTheme("$UBName - Modify Topic \"$PostInfo[0]\"",$HTML);
	exit;
}
###############################################################################
1;# End of ModifyPost Function
###############################################################################