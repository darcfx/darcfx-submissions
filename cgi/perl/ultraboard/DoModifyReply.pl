...
sub DoModifyReply {
	...
	if (($Group ne "administrator")&&($Group ne $BoardInfo[4])) {
        ...
        if ($Group eq "Guest") {
            print "Location: UltraBoard.$Ext?Action=SignIn&Ref=DoModifyReply&Board=$in{'Board'}&Post=$in{'Post'}&ID=$in{'ID'}&Idle=$in{'Idle'}&Sort=$in{'Sort'}&Order=$in{'Order'}&Page=$in{'Page'}\n\n";
...
###############################################################################
	...
	chomp $InLine;
	$POST_DATA[$in{'ID'}+1]=$InLine.$Spliter.&EncodeUBCodes(&RemoveCensorWords($in{'Message'})).$Spliter.++$PostInfo[8]."\n";
...
###############################################################################
	...
	close(BOARD);
###############################################################################
	&ShowThank(	"MODIFY THE MESSAGE",
				"Your message has been modified.\n",
				"3",
...
###############################################################################
1;# End of DoModifyReply Function
###############################################################################