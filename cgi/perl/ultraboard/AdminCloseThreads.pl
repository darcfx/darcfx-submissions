...
sub CloseThreads {
	...
	$HTML.=	&Table($TableWidth,$TableAlign,"","6","","").
                ...
                &Tr("","","").
					&Td("","","","","","","nowrap","","").
						...
						&HiddenBox("Action","DoCloseThreads").
	...
	for ($i=0;$i<=$#SortedList;$i++) {
		...
		if ($PostInfo[9] ne ""){
            $MessageIcon=&Image("$URLImages/LockedMessage.gif","","","","","0",$PostInfo[3]);
        }else{
            $MessageIcon=&Image("$URLImages/OldMessage.gif","","","","","0",$PostInfo[3]);
        }
		$HTML.=	&Tr("","",$RowColor).
	...
	$HTML.=		&Tr("","",$CategoryBGColor).
					&Td("","","5","","","","","","").
						"<center>".&Submit("","CLOSE/OPEN THESE THREADS","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","5","","","","","","").
	...
	($HTML.=<<'			HTML')=~s/^\s+//gm;
			...
			HTML
	&PrintTheme("$UBName Administrative Center - Close/Open Threads",$HTML);
	exit;
}
###############################################################################
1;# End of CloseThreads Function
###############################################################################