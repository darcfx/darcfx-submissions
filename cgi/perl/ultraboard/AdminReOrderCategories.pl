...
###############################################################################
	$HTML.=	&Table($TableWidth,$TableAlign,"","6","","").
                ...
                &Tr("","","").
					&Td("","","","","","","nowrap","","").
						"</form>".
						&Form("UBAdmin.$Ext","POST","","").
						&HiddenBox("Action","DoReOrderCategories").
	...
	for (my ($i)=1;$i<=scalar(@CATEGORIES_DATA);$i++) {
		push (@Select,$i,$i);
	}
	for ($i=0;$i<=$#CATEGORIES_DATA;$i++) {
		...
		$HTML.=	&Tr("","",$RowColor).
					&Td("","","","","","","",$ColumnOddBGColor,"").
						&Select("Category_".$CategoryInfo[0],"","","",$i+1,
							@Select
						).
					"</td>".
	...
	$HTML.=		&Tr("","",$CategoryBGColor).
					&Td("","","4","","","","","","").
						"<center>".&Submit("","REORDER THESE CATEGORIES","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","4","","","","","","").
			...
			"</table></td></tr></table>".
			"</form>";
	&PrintTheme("$UBName Administrative Center - ReOrder Categories",$HTML);
	exit;
}
###############################################################################
1;# End of ReOrderCategories Function
###############################################################################