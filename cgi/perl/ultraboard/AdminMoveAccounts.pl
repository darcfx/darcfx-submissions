...
sub MoveAccounts {
	...
	for (my ($i)=0;$i<=$#GROUPS_DATA;$i++) {
		...
		if ($GroupInfo[0] ne "administrator") {
			push (@GroupList, $GroupInfo[1]." ($GroupInfo[0])", $GroupInfo[0]);
		}
	}
###############################################################################
	...
	$HTML.=	&Table($TableWidth,$TableAlign,"","6","","").
                ...
                &Tr("","","").
					&Td("","","","","","","nowrap","","").
						...
						&HiddenBox("Action","DoMoveAccounts").
	...
	for ($i=0;$i<=$#SortedList;$i++) {
		...
		$HTML.=	&Tr("","",$RowColor).
					&Td("20","","","","CENTER","","",$ColumnOddBGColor,"").
						&Checkbox("MemberID_$i",$MemberInfo[0].$Spliter.$MemberInfo[2],"","").
					"</td>".
	...
	$HTML.=		&Tr("","",$CategoryBGColor).
				...
				"</tr>".
				&Tr("","",$RowOddBGColor).
					&Td("","","6","","","","",$ColumnOddBGColor,"").
						&Select("ToGroup","","width:$IETextBoxSize","","",
							@GroupList
						).			
					"</td>".
				"</tr>".&Tr("","",$CategoryBGColor).
					&Td("","","6","","","","","","").
						"<center>".&Submit("","MOVE THESE MEMBERS ACCOUNT","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","6","","","","","","").
	...
	($HTML.=<<'			HTML')=~s/^\s+//gm;
			...
			HTML
	&PrintTheme("$UBName Administrative Center - Move Members Account",$HTML);
	exit;
}
###############################################################################
1;# End of MoveAccounts Function
###############################################################################