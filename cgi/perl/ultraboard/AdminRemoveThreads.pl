...
sub RemoveThreads {
	...
	$HTML.=	&Table($TableWidth,$TableAlign,"","6","","").
                ...
                &Tr("","","").
					&Td("","","","","","","nowrap","","").
						...
						&HiddenBox("Action","DoRemoveThreads").
	...
	$HTML.=		&Tr("","",$CategoryBGColor).
					&Td("","","5","","","","","","").
						"<center>".&Submit("","REMOVE THESE THREADS","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","5","","","","","","").
	...
	($HTML.=<<'			HTML')=~s/^\s+//gm;
			...
			HTML
	&PrintTheme("$UBName Administrative Center - Remove Threads",$HTML);
	exit;
}
###############################################################################
1;# End of RemoveThreads Function
###############################################################################