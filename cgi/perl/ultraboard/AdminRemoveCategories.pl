...
###############################################################################
	$HTML.=	&Table($TableWidth,$TableAlign,"","6","","").
                ...
                &Tr("","","").
					&Td("","","","","","","nowrap","","").
						...
						&HiddenBox("Action","DoRemoveCategories").
	...
	for (my ($i)=0;$i<=$#CATEGORIES_DATA;$i++) {
		...
		$HTML.=	&Tr("","",$RowColor).
					&Td("20","","","","CENTER","","",$ColumnOddBGColor,"").
						&Checkbox("CategoryID_$i","$CategoryInfo[0]","","").
					"</td>".
	...
	$HTML.=		&Tr("","",$CategoryBGColor).
					&Td("","","4","","","","","","").
						"<center>".&Submit("","REMOVE THESE CATEGORIES","width:$IETextBoxSize")."</center>".
					"</td>".
				"</tr>".
				&Tr("","",$MenuBGColor).
					&Td("","","4","","","","","","").
	...
	($HTML.=<<'			HTML')=~s/^\s+//gm;
			...
			<!--
				function CheckAll () {
					...
					for (var i=0;i<Num;i++) {
						eval ("document.form.CategoryID_"+i+".checked = 1-document.form.CategoryID_"+i+".checked");
					}
				}
			//-->
			</SCRIPT>
			HTML
	&PrintTheme("$UBName Administrative Center - Remove Categories",$HTML);
	exit;
}
###############################################################################
1;# End of RemoveCategories Function
###############################################################################